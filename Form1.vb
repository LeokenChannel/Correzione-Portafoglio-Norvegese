...
Public Class Form1
    Private Sub Btn_Percorso_AnaSog_Completa_Click(sender As Object, e As EventArgs) Handles Btn_Percorso_AnaSog_Completa.Click
        Dim NomeFile As String = Seleziona_Percorso()
    ...
    Function Seleziona_Percorso() As String
        Dim path As String = ""
        ...
        If MessageBox.Show("Si vuole avviare la correzione dell'archivio di Portafoglio selezionato?", Me.Name, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            If ControlloFileDefiniti() = True Then
                If ElaboraCorrezione() = True Then
                    MessageBox.Show("La correzione del Portafoglio indicatoè stata eseguita correttamente", Me.Text)
                End If
            End If
        End If
    ...
    Function ElaboraCorrezione() As Boolean
        Dim EsitoRicomposizione As Boolean = True
        ...
        Try
            ...
            Correggi_Anagist(File_XML_DatiMancanti)


        Catch ex As Exception

            EsitoRicomposizione = False

        End Try

        Return EsitoRicomposizione
    End Function
    ...
    Sub Correggi_Anagist(ByVal File_XML_DatiMancanti As String)
        ...
        Do
            ...
            If TipoSoggetto = "PG" Or TipoSoggetto = "PF" Then
                CFoPIVA = stringReader.Substring(30, 16)                'Codice fiscale o Partita IVA del File NO_... Da controllare.
                ...
                Select Case TipoSoggetto
                    ...
                    Case "PF"                                                           '   Persona Fisica
                        ...
                        stringReader = stringReader.Substring(0, 294) & LuogoNascita & stringReader.Substring(300, stringReader.Length - 300)   'In questo caso è il luogo di nascita
                        'Sostituzione della Sede Legale tramite ricerca in file XML
                        stringReader = stringReader.Substring(0, 309) & SedeLegale & stringReader.Substring(315, stringReader.Length - 315)
                End Select
